def get_file_start_time(file_name: str) -> float:
    """
    :param file_name:
...
def get_file_end_time(file_name: str) -> float:
    """
        :param file_name:
...
def get_file_annotation(file_name: str, start_with_zero: bool) -> list[tuple[str, float, float]]:
    """
    ...
    """
    annotation = []
    file_start_time = get_file_start_time(file_name)
    start = False
    with open(file_name) as file:
        for line in file.readlines():
            ...
            if "" in line and start:
                line_start_time, line_end_time = get_line_time_stamp(line)
                start = False
                if start_with_zero:
...
if __name__ == "__main__":
    print(get_file_end_time("CallHome_eval/transcripts/4093.cha"))
    for line_annote in get_file_annotation("CallHome_eval/transcripts/4093.cha", True):
        print(line_annote)