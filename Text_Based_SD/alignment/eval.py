...
def mapSpeaker(gt_tokens: list[Token], hyp_tokens: list[Token], type):
  ...
  if not hyp_spk2:
    if type == "Rev":
      if hyp_spk1 == rev_spk[0]: hyp_spk2 = rev_spk[1]
      else: hyp_spk2 = rev_spk[0]
...
class Eval_3d:
  ...
  def precision(self): # having two speaker alignment (ground truth to target), find the target to gt alignment and count precision
    ...
    for align in hyp_align:
      if align == 1: correct += 1
      elif align == 0: error += 1
      elif align == -1: gap += 1
      else:
...
def output_3d_eval_csv():
  header = ["file", "error_rate", "correct_rate (recall)", "Gap_rate", "error_count", "correct_count", "Gap", "total_token", "recall", "precision", "F1"]
...
def output_2d_eval_csv():
  header = ["file", "error_rate", "correct_rate (recall)", "Gap_rate","error_count", "correct_count", "Gap", "total_token"]
