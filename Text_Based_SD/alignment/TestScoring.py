...
@jit(nopython=True)
def compare_3d(target_token, token1, token2, tier1, tier2, tier3) -> int:
    mode = 0
    if target_token == '-':
        mode += 4
    if token1 == '-':
...
@jit(nopython=True)
def get_scoring_matrix_3d(target_seq: list[str], seq1: list[str], seq2: list[str], file_code: str, tier1, tier2, tier3) -> ndarray:
    gap = -1
    count = 0
    progress = 0
        ...
        for j in range(1, len(seq1) + 1):
            for k in range(1, len(seq2) + 1):
                if count % int(parameter_number / 100) == 0:
                    ...
                    progress += 1
                spec1 = score[i][j - 1][k - 1] + compare_3d('-', seq1[j - 1], seq2[k - 1], tier1, tier2, tier3)
                spec2 = score[i - 1][j][k - 1] + compare_3d(target_seq[i - 1], '-', seq2[k - 1], tier1, tier2, tier3)
                spec3 = score[i - 1][j - 1][k] + compare_3d(target_seq[i - 1], seq1[j - 1], '-', tier1, tier2, tier3)
                spec4 = score[i - 1][j][k] + compare_3d(target_seq[i - 1], '-', '-', tier1, tier2, tier3)
                spec5 = score[i - 1][j][k - 1] + compare_3d('-', seq1[j - 1], '-', tier1, tier2, tier3)
                spec6 = score[i][j][k - 1] + compare_3d('-', '-', seq2[k - 1], tier1, tier2, tier3)
                spec7 = score[i - 1][j - 1][k - 1] + compare_3d(target_seq[i - 1], seq1[j - 1], seq2[k - 1], tier1, tier2, tier3)
...
@jit(nopython=True)
def backtrack(target_seq, seq1, seq2, matrix, file_code: str, tier1, tier2, tier3):
    gap = -1
    i = len(target_seq)
    j = len(seq1)
    k = len(seq2)
    ...
    while i > 0 and j > 0 and k > 0:
        ...
        yj = seq1[j - 1]
        zk = seq2[k - 1]
        if matrix[i, j, k] == compare_3d('-', yj, zk, tier1, tier2, tier3) + matrix[i, j - 1, k - 1]:
            align1.append('-')
            align2.append(yj)
            ...
            j -= 1
            k -= 1
        elif matrix[i, j, k] == compare_3d(xi, '-', zk, tier1, tier2, tier3) + matrix[i - 1, j, k - 1]:
            align1.append(xi)
            align2.append('-')
            align3.append(zk)
            ...
            i -= 1
            k -= 1
        elif matrix[i, j, k] == compare_3d(xi, yj, '-', tier1, tier2, tier3) + matrix[i - 1, j - 1, k]:
            align1.append(xi)
            align2.append(yj)
            ...
            i -= 1
            j -= 1
        elif matrix[i, j, k] == compare_3d(xi, '-', '-', tier1, tier2, tier3) + matrix[i - 1, j, k]:
            align1.append(xi)
            align2.append('-')
            align3.append('-')
            i -= 1
        elif matrix[i, j, k] == compare_3d('-', yj, '-', tier1, tier2, tier3) + matrix[i, j - 1, k]:
            align1.append('-')
            align2.append(yj)
            align2_to_align1[j] = -1
            align3.append('-')
            j -= 1
        elif matrix[i, j, k] == compare_3d('-', '-', zk, tier1, tier2, tier3) + matrix[i, j, k - 1]:
            align1.append('-')
            align2.append('-')
            align3.append(zk)
            align3_to_align1[k] = -1
            k -= 1
        elif matrix[i, j, k] == compare_3d(xi, yj, zk, tier1, tier2, tier3) + matrix[i - 1, j - 1, k - 1]:
            align1.append(xi)
            align2.append(yj)
            align2_to_align1[j] = i
            align3.append(zk)
...
def write_csv(tier_list: list[int]):
    file_code = "4074"
    ...
    align1, align2, align3, align2_to_align1, align3_to_align1 = \
        backtrack(target, seq1, seq2,
                  get_scoring_matrix_3d(target, seq1, seq2, file_code, tier_list[0], tier_list[1], tier_list[2]),
                  file_code, tier_list[0], tier_list[1], tier_list[2])
    with open(f"../alignment/ResultRevAI/TestScoringResult/{file_code}_{tier_list[0]}_{tier_list[1]}_{tier_list[2]}.csv", 'w') as file:
        output = csv.writer(file)
        output.writerows([align2, align3, align1, align2_to_align1, align3_to_align1])
    print(f"{file_code}_{tier_list[0]}_{tier_list[1]}_{tier_list[2]}.csv has been written.\n")
...
if __name__ == "__main__":
    ...
    parameter_list = [[-i, 0, i] for i in range(1, 6)]
    with Pool(5) as pool:
        pool.map(write_csv, parameter_list)
